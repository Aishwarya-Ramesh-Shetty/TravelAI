...
exports.uploadAndExtract = async (req, res) => {
  try {
    ...
    const extractedData = await aiService.extractDataFromText(combinedText);
...
exports.createTrip = async (req, res) => {
  try {
    const { extractedData, destination, startDate, endDate } = req.body;
    
    const itinerary = await aiService.generateItinerary({
      destination, startDate, endDate, ...extractedData
    });
